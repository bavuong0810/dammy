...
@foreach ($authors as $author)
    ...
    if(isset($author->avatar) && $author->avatar !=''):
        $img_thumb = asset('images/avatar/' . $author->avatar);
    else:
        $img_thumb = asset($settings->where('type', 'seo_image')->first()->value);
    endif;
    $xml_file .='
    <url>
    <loc>' . route('translateTeam.detail', $author->id) . '</loc>
    <image:image>
    <image:loc>' . $img_thumb . '</image:loc>
    <image:caption>' . $author->name . '</image:caption>
    <image:license>' . route('index') . '</image:license>
    <image:family_friendly>yes</image:family_friendly>
    </image:image>
    <lastmod>' . date("Y-m-d", strtotime($author->created_at)) . '</lastmod>
