...
class AdminController extends Controller
{
    ...
    public function storePassword(Request $rq)
    {
        ...
        $response = Admin::where("id", $id)->update($data);
    ...
    public function storeThemeOption(Request $request)
    {
        $array_option_tdr = array();
        if (isset($request->header_option_values_line) && !empty($request->header_option_values_line) && $request->header_option_values_line != ''):
            for ($i = 0; $i < count($request->header_option_values_line); $i++):
                $header_option_texts_line = ($request->header_option_texts_line[$i] != '') ? $request->header_option_texts_line[$i] : '';
                $header_option_values_line = ($request->header_option_values_line[$i] != '') ? $request->header_option_values_line[$i] : '';
        ...
        if (isset($request->header_option_values_muti_line) && !empty($request->header_option_values_muti_line) && $request->header_option_values_muti_line != ''):
            for ($i = 0; $i < count($request->header_option_values_muti_line); $i++):
                $header_option_texts_muti_line = ($request->header_option_texts_muti_line[$i] != '') ? $request->header_option_texts_muti_line[$i] : '';
                $header_option_values_muti_line = ($request->header_option_values_muti_line[$i] != '') ? $request->header_option_values_muti_line[$i] : '';
